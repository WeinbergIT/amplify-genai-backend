...
def get_api_key(event, context, user, name, data):
    ...
    try:
        ...
        response = table.get_item(Key={"api_owner_id": api_key_id})
        ...
        if "Item" in response:
            ...
            if (item["owner"] == user and not delegate) or (delegate == user):
                ...
                message = (
                    ...
                    else "The key has been deactivated"
                )
        ...
        if "Items" in response and response["Items"]:
            ...
            for item in response["Items"]:
                ...
                if item.get("active") and expiration and is_expired(expiration):
                    print(
                        f"Key {item.get('applicationName')} is expired and will be deactivated "
                    )
...
def create_api_keys(event, context, user, name, data):
    ...
    if not can_create["success"]:
        return {"success": False, "message": can_create["message"]}
