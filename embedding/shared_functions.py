...
def generate_bedrock_embeddings(content):
    try:
        ...
        logger.info(
            f"Embedding generated. Input tokens: {input_token_count}, Embedding size: {len(embeddings)}"
        )
    ...
    try:
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "With every prompt I send, think about what questions the text might be able to answer and return those questions. Please create many questions.",
                },
        ...
        return {
            "success": False,
            "error": f"An error occurred with Azure OpenAI: {str(e)}",
        }
...
def record_usage(
    ...
    try:
        ...
        response = dynamodb.put_item(TableName=dynamo_table_name, Item=item)
        ...
        if status_code == 200:
            ...
            logger.info(
                f"Token Usage recorded for embedding source {text_location_key}, to {account} of {original_creator}."
            )
        else:
