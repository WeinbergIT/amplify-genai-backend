    ...
    with psycopg2.connect(
        ...
        with conn.cursor() as cur:
            ...
            try:
                ...
                logger.error(
                    f"An error occurred while fetching top similar docs: {e}",
                    exc_info=True,
                )
                raise
    return top_docs
...
def classify_src_ids_by_access(raw_src_ids, current_user):
    ...
    try:
        ...
        logger.info(
            f"Accessible src_ids: {accessible_src_ids}, Access denied src_ids: {access_denied_src_ids}"
        )
...
def classify_group_src_ids_by_access(raw_group_src_ids, current_user):
    ...
    print(
        f"Group Accessible src_ids: {accessible_src_ids}, Group Access denied src_ids: {access_denied_src_ids}"
    )
    return accessible_src_ids, access_denied_src_ids
...
def process_input_with_dual_retrieval(event, context, current_user, name, data):
    ...
    group_accessible_src_ids, group_access_denied_src_ids = (
        classify_group_src_ids_by_access(raw_group_src_ids, current_user)
    )
