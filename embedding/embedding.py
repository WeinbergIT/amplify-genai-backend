...
def update_child_chunk_status(object_id, child_chunk, new_status):
    try:
        ...
        print(
            f"Updating status of child chunk {child_chunk} for {object_id} to {new_status}"
        )
...
def update_parent_chunk_status(object_id):
    ...
    try:
        ...
        if all_complete:
            logging.info(
                "Updating parentChunkStatus to completed for object_id: %s", object_id
            )
            table.update_item(
                Key={"object_id": object_id},
            ...
            logging.info(
                "parentChunkStatus updated to completed for object_id: %s", object_id
            )
        else:
            logging.info(
                "Not all child chunks are complete for object_id: %s", object_id
            )
    except Exception as e:
...
def lambda_handler(event, context):
    ...
    for record in event["Records"]:
        ...
        try:
            ...
            success, src = embed_chunks(
                data, childChunk, embedding_progress_table, db_connection
            )
    ...
    try:
        ...
        try:
            ...
            if item and "data" in item:
                ...
                logging.info(
                    f"Processing child chunk: {childChunk} of total parent chunks: {total_chunks}"
                )
                local_chunks_to_process = len(local_chunks)
                logging.info(
                    f"There are {local_chunks_to_process} (max 10) within child chunk: {childChunk}"
                )
        ...
        logging.info(
            f"Processing {childChunk} of {total_chunks} (fetched from DynamoDB)"
        )
        ...
        with db_connection.cursor() as cursor:
            ...
            for local_chunk_index, chunk in enumerate(
                ...
                try:
                    ...
                    response_vector_embedding = generate_embeddings(
                        clean_text, embedding_provider
                    )
                    ...
                    response_qa_summary = generate_questions(
                        clean_text, embedding_provider
                    )
                    ...
                    response_qa_embedding = generate_embeddings(
                        content=qa_summary, embedding_provider=embedding_provider
                    )
                    ...
                    if result:
                        ...
                        logging.info(
                            f"Account: {account}, User: {user}, API Key: {api_key}"
                        )
                    else:
                        logging.error(
                            f"Failed to retrieve account details for {trimmed_src}"
                        )
                    ...
                    try:
                        record_usage(
                            ...
                            qa_model_name,
                            api_key=api_key,
                            input_tokens=qa_summary_input_tokens,
                            output_tokens=None,
                        )
                        logging.info(
                            f"Successfully recorded usage for qa_model_name input tokens. Account: {account}, User: {user}"
                        )
                    except Exception as e:
                        logging.error(
                            f"Error recording usage for qa_model_name input tokens: {str(e)}"
                        )
                    ...
                    try:
                        record_usage(
                            ...
                            api_key=api_key,
                            input_tokens=None,
                            output_tokens=qa_summary_output_token_count,
                        )
                        logging.info(
                            f"Successfully recorded usage for qa_model_name output tokens. Account: {account}, User: {user}"
                        )
                    except Exception as e:
                        logging.error(
                            f"Error recording usage for qa_model_name output tokens: {str(e)}"
                        )
                        logging.exception("Full traceback:")
                    ...
                    try:
                        record_usage(
                            ...
                            api_key=api_key,
                            output_tokens=total_vector_token_count,
                            input_tokens=None,
                        )
                        logging.info(
                            f"Successfully recorded usage for embedding_model_name. Account: {account}, User: {user}"
                        )
                    except Exception as e:
                        logging.error(
                            f"Error recording usage for embedding_model_name: {str(e)}"
                        )
                        logging.exception("Full traceback:")
                    ...
                    logging.error(
                        f"Error processing chunk {local_chunk_index} of {src}: {str(e)}"
                    )
...
def terminate_embedding(event, context, current_user, name, data):
    ...
    try:
        ...
        print(
            f"Error terminating embedding for object_id {object_id}: {e}", exc_info=True
        )
        return False
