...
def save_chunks(chunks_bucket, key, split_count, chunks):
    print(
        f"Saving chunks {len(chunks)} to {chunks_bucket}/{key}-{split_count}.chunks.json"
    )
...
def decrypt_account_data(encrypted_data_b64):
    ...
    try:
        ...
        print(
            f"Error during parameter retrieval or encryption {parameter_name}: {str(e)}"
        )
...
def update_embedding_status(
    original_creator, object_id, chunk_index, total_chunks, status
):
    try:
