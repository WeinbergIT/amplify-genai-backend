...
"""
    ...
    print(
        f"Returning presigned URL for key: {s3_output_key} and bucket: {output_bucket_name}"
    )
...
def handler(event, context):
    ...
    for record in event["Records"]:
        ...
        print(
            f"bucket:{input_bucket} user: {email}, uuid: {uuid}, format: {fmat}, extension: {extension}"
        )
        ...
        print(
            f"input_bucket: {input_bucket}, key: {key}, input_file: {input_file.name}, output_file: {output_file.name}, mime_type: {mime_type}"
        )
