...
def sanity_check_dsl(dsl):
    ...
    for query in dsl_dict["queries"]:
        ...
        if query["hashKey"] not in dsl_dict["schema"]:
            raise ValueError(
                f"hashKey {query['hashKey']} in queries is not defined in the schema"
            )

        # If rangeKey is present, check it in schema
        if "rangeKey" in query and query["rangeKey"] not in dsl_dict["schema"]:
            raise ValueError(
                f"rangeKey {query['rangeKey']} in queries is not defined in the schema"
            )
...
def update_yml(serveryml, dsl):
    ...
    keys.update(
        query["rangeKey"] for query in dsl_dict["queries"] if "rangeKey" in query
    )
