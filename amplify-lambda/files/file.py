...
def get_presigned_download_url(event, context, current_user, name, data):
    ...
    if response["Item"]["createdBy"] == group_id:
        ...
        if group_id:  # checks can access of the group_id
            ...
            print(
                f"User {current_user} does not have acces to download: {response['Item']}"
            )
            return {
                "success": False,
...
def create_and_store_fernet_key():
    ...
    if not parameter_name:
        raise ValueError(
            "The environment variable FILE_UPLOAD_ENCRYPTION_PARAMETER is not set"
        )
...
def encrypt_account_data(data):
    ...
    try:
        ...
        return {"success": True, "encrypted_data": encrypted_data_b64}
...
def get_presigned_url(event, context, current_user, name, data):
    ...
    bucket_name, key = create_file_metadata_entry(
        current_user, name, file_type, tags, props, knowledge_base
    )
    ...
    print(
        f"Getting presigned URL for text content {text_content_key} in bucket {file_text_content_bucket_name}"
    )
...
@op(
    ...
    """,
    params={"tag": "String. Required. The tag to be deleted. Example: 'NewTag'."},
)
@op(
    path="/files/tags/delete",
...
@op(
    ...
    params={"tags": "A list of string tags to create for the user."},
)
@validated(op="create")
...
@op(
    ...
    tags=["files"],
    description="Search a user's list of files with a query.",
    params={"namePrefix": "The prefix to search for in the file names."},
)
@op(
    path="/files/query",
