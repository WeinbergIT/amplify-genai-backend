...
def optimize(event, context, current_user, name, data):
    try:
        ...
        result = prompt_generator(
            task=idea, model=model, access_token=access_token, chat_url=chat_url
        )
        ...
        print("Error: ", e)
        return {"success": False, "message": "Failed to generate an optimized prompt."}


class PromptInput(BaseModel):
    task: str = Field(description="The task to generate a prompt template for.")
    max_placeholders: int = Field(
        description="The maximum number of placeholders to use in the prompt template."
    )
